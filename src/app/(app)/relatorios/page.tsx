...
export default function Relatorios() {
  ...
  const FilterBtn = ({label, active, onClick}: {label:string;active:boolean;onClick:()=>void}) => (
    <button onClick={onClick} style={{
      ...
      border: `1px solid ${active ? TERRA : 'rgba(0,0,0,0.08)'}`,
    ...
    <div style={{ background:BG, minHeight:'100%', padding:'14px 14px 160px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <button onClick={()=>setDate(d=>subMonths(d,1))} style={{ width:32,height:32,background:'rgba(0,0,0,0.04)',borderRadius:12,border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12 }}>
        <button onClick={()=>setModal({title:'Receitas',txs:mes.filter((t:any)=>t.transaction_type==='receita')})}
          style={{ background:'rgba(52,199,89,0.06)', borderRadius:20, padding:'14px 16px', border:'0.5px solid rgba(74,140,92,0.35)', textAlign:'left', cursor:'pointer' }}>
          ...
          <p style={{ fontSize:22, fontWeight:800, color:'#34C759', margin:'0 0 4px', fontVariantNumeric:'tabular-nums' }}>
        ...
        <button onClick={()=>setModal({title:'Despesas',txs:mes.filter((t:any)=>t.transaction_type!=='receita')})}
          style={{ background:'rgba(255,59,48,0.06)', borderRadius:20, padding:'14px 16px', border:'0.5px solid rgba(196,98,45,0.35)', textAlign:'left', cursor:'pointer' }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:8 }}>
            <TrendingDown size={14} color='#FF3B30'/>
            <p style={{ fontSize:11, color:'rgba(255,138,92,0.7)', margin:0, fontWeight:600 }}>Despesas</p>
          </div>
          <p style={{ fontSize:22, fontWeight:800, color:'#FF3B30', margin:'0 0 4px', fontVariantNumeric:'tabular-nums' }}>
      ...
      <div style={{ ...pebble(), display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          ...
          <p style={{ fontSize:24, fontWeight:800, color: saldo>=0 ? '#34C759' : '#FF3B30', margin:0, fontVariantNumeric:'tabular-nums' }}>
            {saldo>=0?'+':''}{formatCurrency(saldo)}
          </p>
        </div>
        <div style={{ textAlign:'right' }}>
          <p style={{ fontSize:10, color:TEXTMU, margin:'0 0 2px' }}>Taxa de poupança</p>
          <p style={{ fontSize:16, fontWeight:700, color: receitas>0&&saldo>0 ? '#34C759' : TEXTMU, margin:0 }}>
      ...
      <div style={{ ...pebble() }}>
          ...
          <div style={{ display:'flex', gap:6 }}>
            {[3,4,6].map(n => (
              <button key={n} onClick={()=>setCompN(n)} style={{
                ...
                background: compN===n ? TERRA : 'rgba(0,0,0,0.04)',
        ...
        <div style={{ display:'flex', gap:16, marginBottom:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <div style={{ width:10, height:10, borderRadius:3, background:'#34C759' }}/>
            <span style={{ fontSize:11, color:TEXTLT }}>Receitas</span>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <div style={{ width:10, height:10, borderRadius:3, background:'#FF3B30' }}/>
          ...
          {comp.map((c, i) => (
            <div key={i}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                ...
                <span style={{ fontSize:11, color: c.d > c.r ? '#FF3B30' : '#34C759', fontWeight:600, fontVariantNumeric:'tabular-nums' }}>
                  {formatCurrency(c.d)}
                </span>
              </div>
              {/* Barra receita */}
              <div style={{ height:7, background:'rgba(0,0,0,0.04)', borderRadius:99, overflow:'hidden', marginBottom:3 }}>
                <div style={{
                  height:'100%', borderRadius:99,
                  width:`${(c.r/maxComp)*100}%`,
                  background: c.atual ? '#34C759' : 'rgba(93,224,138,0.4)',
                  transition:'width 0.5s ease',
                  minWidth: c.r>0 ? 4 : 0,
                }}/>
              </div>
              {/* Barra despesa */}
              <div style={{ height:7, background:'rgba(0,0,0,0.04)', borderRadius:99, overflow:'hidden' }}>
                <div style={{
                  height:'100%', borderRadius:99,
                  width:`${(c.d/maxComp)*100}%`,
                  background: c.atual ? '#FF3B30' : 'rgba(255,138,92,0.4)',
                  transition:'width 0.5s ease',
                  minWidth: c.d>0 ? 4 : 0,
                }}/>
              </div>
              ...
              {hist.map((h, i) => (
                <div key={i}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                    ...
                    <span style={{ fontSize:12, color: h.atual ? '#FF3B30' : TEXTLT, fontWeight:600, fontVariantNumeric:'tabular-nums' }}>
                  ...
                  <div style={{ height:8, background:'rgba(0,0,0,0.04)', borderRadius:99, overflow:'hidden' }}>
                    <div style={{
                      ...
                      background: h.atual ? '#FF3B30' : 'rgba(255,138,92,0.45)',
                      transition:'width 0.5s ease',
      ...
      {modal && (
        <div style={{ position:'fixed',inset:0,zIndex:60,display:'flex',alignItems:'flex-end' }} onClick={()=>setModal(null)}>
          ...
          <div style={{ position:'relative',width:'100%',maxWidth:480,margin:'0 auto',background:'#FFFFFF',borderRadius:'28px 28px 0 0',maxHeight:'82vh',display:'flex',flexDirection:'column',boxShadow:'0 -8px 40px rgba(0,0,0,0.6)' }} onClick={e=>e.stopPropagation()}>
            <div style={{ padding:'18px 20px 14px',borderBottom:'1px solid rgba(0,0,0,0.06)',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'space-between' }}>
              <div>
            ...
            <div style={{ overflowY:'auto',flex:1,padding:'12px 20px 24px' }}>
              {modal.txs.map((t:any,i:number) => (
                <div key={t.id} style={{ display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderTop:i>0?'1px solid rgba(0,0,0,0.04)':undefined }}>
                  ...
                  <p style={{ fontSize:13,fontWeight:700,fontVariantNumeric:'tabular-nums',margin:0,color:t.transaction_type==='receita'?'#34C759':'#FF3B30',flexShrink:0 }}>
