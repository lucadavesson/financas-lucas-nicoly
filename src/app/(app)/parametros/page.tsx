...
const TEXT='#1C1C1E'; const TEXTMU='#8E8E93'; const TERRA='#C4622D'; const CREAM='#FFFFFF'
...
export default function Parametros() {
  ...
  const inp = {width:'100%',height:46,background:'rgba(0,0,0,0.03)',border:'0.5px solid rgba(0,0,0,0.08)',borderRadius:22,padding:'0 16px',fontSize:14,color:'#F4EFE8',outline:'none',boxSizing:'border-box' as const}
  const seg = (on:boolean)=>({flex:1,height:38,borderRadius:19,border:'none',background:on?TERRA:'rgba(0,0,0,0.03)',color:on?CREAM:TEXT,fontSize:13,fontWeight:on?600:400 as any,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'})

  if (sec==='cartoes') return (
    <div style={{ background:BG,minHeight:'100%',padding:'14px 14px 160px' }}>
      <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:16 }}>
        <button onClick={()=>setSec('main')} style={{ background:'rgba(0,0,0,0.03)',border:'none',borderRadius:12,padding:'6px 12px',cursor:'pointer',fontSize:13,color:TEXT,fontWeight:600 }}>← Voltar</button>
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
        {cards.map(c=>(
          <div key={c.id} style={{ background:SEBBLE,borderRadius:24,padding:'14px 16px',display:'flex',alignItems:'center',gap:12,boxShadow:'0 3px 10px rgba(0,0,0,0.04),inset 0 1px 0 rgba(255,255,255,0.25)',opacity:c.is_active?1:0.5 }}>
      ...
      {showC&&(
        <div style={{ position:'fixed',inset:0,zIndex:60,display:'flex',alignItems:'flex-end' }} onClick={()=>setShowC(false)}>
          ...
          <div style={{ position:'relative',width:'100%',maxWidth:480,margin:'0 auto',background:'#FFFFFF',borderRadius:'32px 32px 0 0',maxHeight:'92vh',display:'flex',flexDirection:'column' }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ overflowY:'auto',overscrollBehavior:'none',flex:1,background:'#FFFFFF' }}>
              <form onSubmit={saveCard} style={{ padding:'16px 20px 60px',background:'#FFFFFF',display:'flex',flexDirection:'column',gap:14 }}>
                {/* Preview */}
                <div style={{ borderRadius:20,padding:'16px',background:prevColor,position:'relative',overflow:'hidden' }}>
                  <div style={{ position:'absolute',right:-10,top:-10,width:70,height:70,borderRadius:'50%',background:'rgba(0,0,0,0.08)' }}/>
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
        {[
          ...
          <button key={item.s} onClick={()=>item.s==='cartoes'?setSec('cartoes'):toast.info('Em breve!')}
            ...
            <div style={{ width:44,height:44,background:'rgba(0,0,0,0.03)',borderRadius:15,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
