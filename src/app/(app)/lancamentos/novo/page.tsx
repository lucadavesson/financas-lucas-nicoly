    ...
    try {
      if (tipo === 'parcelada') {
        ...
        const { error } = await supabase.from('transactions').insert({
          owner_id:user.id, owner_name:ownerName, holder, transaction_type:'parcelada',
        ...
        if (hasEntry && entryAmt > 0) {
          ...
          await supabase.from('transactions').insert({
            owner_id:user.id, owner_name:ownerName, holder, transaction_type:'avista',
        ...
        const { error } = await supabase.from('transactions').insert({
          owner_id:user.id, owner_name:ownerName, holder, transaction_type:'recorrente',
        ...
        const { error } = await supabase.from('transactions').insert({
          owner_id:user.id, owner_name:ownerName, holder, transaction_type:'receita',
