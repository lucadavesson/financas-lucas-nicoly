...
export default function NovoLancamento() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const ownerName = (holder === 'Prata' ? 'Lucas' : holder) || 'Lucas'
      ...
      if (tipo === 'parcelada') {
        ...
        if (hasEntry && entryAmt > 0) {
          ...
          await supabase.from('transactions').insert({
            owner_id:user.id, owner_name:ownerName, holder, transaction_type:'avista', type:'Despesa',
        ...
        const { error } = await supabase.from('transactions').insert({
          owner_id:user.id, owner_name:ownerName, holder, transaction_type:'recorrente', type:'Despesa',
        ...
        const { error } = await supabase.from('transactions').insert({
          owner_id:user.id, owner_name:ownerName, holder, transaction_type:'receita', type:'Receita',
