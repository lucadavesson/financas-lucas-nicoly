...
import { Home, List, Plus, CreditCard, BarChart2, Wallet } from 'lucide-react'
...
export default function BottomNav() {
  ...
  return (
    <div style={{ position:'fixed', bottom:0, left:'50%', transform:'translateX(-50%)', width:'100%', maxWidth:480, zIndex:50 }}>
      ...
      <nav style={{ background:'linear-gradient(180deg,#2A1C0E,#1A1208)', display:'flex', alignItems:'flex-end', paddingBottom:'env(safe-area-inset-bottom,12px)', paddingTop:2 }}>
        {items.map((item) => {
          if (item.fab) return (
          ...
          return (
            <Link key={item.href} href={item.href!} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3, padding:'4px 4px 8px', textDecoration:'none' }}>
